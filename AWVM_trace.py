...
def register_video_entry(x, y, zoom, address):
  ...
  if address in video_entries.keys():
    return video_entries[address]["label"]
...
class AWVM_Trace(ExecTrace):
  ...
  def disasm_instruction(self, opcode):
    if (opcode & 0x80) == 0x80:  # VIDEO
      ...
      label = register_video_entry(x, y, "0x40", offset)
      return "video: off=%s x=%d y=%d" % (label, x, y)
      ...
      if opcode & 3 == 3:
        ...
        #print("found video_entry {} at PC={}".format(hex(offset), hex(self.PC)))
        label = register_video_entry(x_str, y_str, zoom_str, offset)

      return "video: off=%s x=%s y=%s zoom:%s" % (label, x_str, y_str, zoom_str)
