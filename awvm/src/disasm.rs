...
impl<'v> AwvmDisassembler<'v> {
    ...
    /// Stage-aware variable name. Consults the current release's
    /// `stage_specific_vars` table for the current `game_level`
    /// first; falls back to the global `SPECIAL_PURPOSE_VARS`
    /// table; last resort is the literal `[0xNN]` form.
    fn variable_name(&self, value: u8) -> String {
        for (lvl, aliases) in self.release.stage_specific_vars {
...
impl<'v> AwvmDisassembler<'v> {
    fn do_disasm_instruction(&mut self, t: &mut Tracer, opcode: u8) -> String {
        ...
        match opcode {
            0x00 => {
                let dst_var = self.variable_name(t.fetch());
                let imm_hi = t.fetch();
                let imm_lo = t.fetch();
            ...
            0x01 => {
                let dst_var = self.variable_name(t.fetch());
                let src_var = self.variable_name(t.fetch());
            ...
            0x0a => {
                ...
                let b = t.fetch();
                let c = t.fetch();
                let var1 = self.variable_name(b);
                let midterm: String;
                if subop & 0x80 != 0 {
                    let var2 = self.variable_name(c);
                    midterm = format!("[{}]", var2);
            ...
            0x13 => {
                let v1 = self.variable_name(t.fetch());
                let v2 = self.variable_name(t.fetch());
                format!("sub [{}], [{}]", v1, v2)
            }
            0x14 => {
                let dst = self.variable_name(t.fetch());
                let hi = t.fetch();
                let lo = t.fetch();
            ...
            0x16 => {
                let var = self.variable_name(t.fetch());
                let hi = t.fetch();
                let lo = t.fetch();
                let imm = (u32::from(hi) << 8) | u32::from(lo);
